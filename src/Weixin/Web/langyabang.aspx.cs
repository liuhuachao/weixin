...
{
    public partial class langyabang : System.Web.UI.Page
    {
        LogHelper log = new LogHelper("琅琊榜页面日志");
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (string.IsNullOrEmpty(code))
                {
                    ...
                    Response.Redirect(string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri=http%3a%2f%2fweixin.luxlead.com%2fWeb%2flangyabang.aspx&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect ", appid));
                    return;
                }
                else
                {
