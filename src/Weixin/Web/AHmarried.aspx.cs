...
{
    public partial class AHmarried : System.Web.UI.Page
    {
        LogHelper log = new LogHelper("黄晓明结婚页面日志");
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (string.IsNullOrEmpty(code))
                {
                    ...
                    Response.Redirect(string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri=http%3a%2f%2fweixin.luxlead.com%2fWeb%2fAHmarried.aspx&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect ", appid));
                    return;
                }
                else
                {
